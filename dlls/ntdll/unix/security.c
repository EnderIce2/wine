...
NTSTATUS WINAPI NtOpenProcessTokenEx(HANDLE process, DWORD access, DWORD attributes, HANDLE *handle)
{
    ...
    SERVER_START_REQ(open_token)
    {
        ...
        if (!ret)
            *handle = wine_server_ptr_handle(reply->token);
    }
    SERVER_END_REQ;
    return ret;
}
                                 ...
                                 SECURITY_IMPERSONATION_LEVEL level, TOKEN_TYPE type, HANDLE *handle)
{
    ...
    if ((status = alloc_object_attributes(attr, &objattr, &len)))
        return status;
    ...
    SERVER_START_REQ(duplicate_token)
    {
        ...
        if (!status)
            *handle = wine_server_ptr_handle(reply->new_handle);
    }
    SERVER_END_REQ;
    free(objattr);
    return status;
...
static BOOL once_NtQueryInformationToken;
                                        ...
                                        void *info, ULONG length, ULONG *retlen)
{
    ...
    case TokenUser:
        SERVER_START_REQ(get_token_sid)
        {
            ...
            status = wine_server_call(req);
            if (retlen)
                *retlen = reply->sid_len + sizeof(TOKEN_USER);
    ...
    case TokenPrimaryGroup:
        SERVER_START_REQ(get_token_sid)
        {
            ...
            status = wine_server_call(req);
            if (retlen)
                *retlen = reply->sid_len + sizeof(TOKEN_PRIMARY_GROUP);
            if (status == STATUS_SUCCESS)
    ...
    case TokenOwner:
        SERVER_START_REQ(get_token_sid)
        {
            ...
            status = wine_server_call(req);
            if (retlen)
                *retlen = reply->sid_len + sizeof(TOKEN_OWNER);
            if (status == STATUS_SUCCESS)
                towner->Owner = sid;
        }
        SERVER_END_REQ;
    ...
    case TokenType:
        SERVER_START_REQ(get_token_info)
        {
            ...
            status = wine_server_call(req);
            if (status == STATUS_SUCCESS)
                *type = reply->primary ? TokenPrimary : TokenImpersonation;
        }
        SERVER_END_REQ;
        break;
    ...
    case TokenDefaultDacl:
        SERVER_START_REQ(get_token_default_dacl)
        {
            ...
            status = wine_server_call(req);
            if (retlen)
                *retlen = reply->acl_len + sizeof(TOKEN_DEFAULT_DACL);
            if (status == STATUS_SUCCESS)
            {
                if (reply->acl_len)
    ...
    case TokenIntegrityLevel:
    {
        ...
        static const SID high_level = {SID_REVISION, 1, {SECURITY_MANDATORY_LABEL_AUTHORITY}, {SECURITY_MANDATORY_HIGH_RID}};
    ...
    case TokenLinkedToken:
        SERVER_START_REQ(create_linked_token)
        {
            ...
            status = wine_server_call(req);
            if (!status)
                linked->LinkedToken = wine_server_ptr_handle(reply->linked);
        }
        SERVER_END_REQ;
        break;
                                        ...
                                        DWORD length, TOKEN_PRIVILEGES *prev, DWORD *retlen)
{
    ...
    SERVER_START_REQ(adjust_token_privileges)
    {
        ...
        {
            if (retlen)
                *retlen = reply->len + FIELD_OFFSET(TOKEN_PRIVILEGES, Privileges);
                              ...
                              TOKEN_PRIVILEGES *privileges, TOKEN_GROUPS *restrict_sids, HANDLE *new_token)
{
    ...
    SERVER_START_REQ(filter_token)
    {
        ...
        status = wine_server_call(req);
        if (!status)
            *new_token = wine_server_ptr_handle(reply->new_handle);
    }
    SERVER_END_REQ;
...
NTSTATUS SepImpersonateAnonymousToken(PETHREAD thread, KPROCESSOR_MODE PreviousMode)
{
    FIXME("(%p, %c): stub\n", thread, PreviousMode);
    return STATUS_NOT_IMPLEMENTED;
    // NTSTATUS status;
